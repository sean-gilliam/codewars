...
public long Thirt(long number)
{
    var pattern = new[] {1, 10, 9, 12, 3, 4};
    Func<long, char[]> split = x => x.ToString().Reverse().ToArray();
    var num = split(number);
      
    var nums = new List<long>();
    long found = 0;
    long calculate(char[] n)
    {
        var sum = Enumerable.Range(0, n.Length)
            .Select(x => Convert.ToInt64(n[x].ToString()) * pattern[x % pattern.Length])
            .Sum();
        nums.Add(sum);
