...
public string DuplicateEncode(string word)
{
	...
	Func<char, char, bool> cmp = (x, y) => string.Compare(x.ToString(), y.ToString(), StringComparison.OrdinalIgnoreCase) == 0;
	return string.Join(string.Empty, word.Select(x => word.Count(y => cmp(x, y)) == 1 ? "(" : ")"));
}
